...
def market_decision(nifty, score):
    ...
    # ATR must be valid before any risk/target levels are generated.
    ...
    return (
        ...
        trigger_level if decision != "WAIT" else np.nan,
        reason
    )
...
def signal_details(
    ...
    # R:R uses the current/planned spot price, which is the midpoint
    # of the symmetric ATR entry zone.
    if decision == "BUY":
        strength = "STRONG BUY" if score >= 75 else "MODERATE BUY"
        trigger = f"BUY confirmation: NIFTY closes above {trigger_level:,.0f}"
        risk = spot - sl
        reward_t1 = t1 - spot
        reward_t2 = t2 - spot
        ...
        trigger = f"SHORT confirmation: NIFTY closes below {trigger_level:,.0f}"
        risk = sl - spot
        reward_t1 = spot - t1
        reward_t2 = spot - t2

...
def regime_score(nifty, bank, ivr, vrpcur, hcur, corrcur, lreg):
    """Directional 0-100 score using trend + microstructure, with volatility context."""
    n=series(nifty,"Close"); b=series(bank,"Close")
    ...
    # NIFTY trend: 40 points
    trend_points=0
    trend_points += 15 if n.iloc[-1] > ne20 else 0
    trend_points += 15 if ne20 > ne50 else 0
    trend_points += 10 if n.iloc[-1] > ne50 else 0

    # BANK NIFTY confirmation: 15 points
    bank_points=(10 if b.iloc[-1] > be20 else 0) + (5 if be20 > be50 else 0)

    # Correlation/confirmation: 10 points. Strong correlation is supportive; low correlation is neutral-to-negative.
    corr_points=10 if corrcur >= .80 else (5 if corrcur >= .50 else 2)

    # Liquidity microstructure: 15 points
    liq_points=15 if lreg=="DEMAND LIQUIDITY SWEPT" else (0 if lreg=="SUPPLY LIQUIDITY SWEPT" else 8)

    # Hurst is used as regime quality, not as a direct bullish/bearish signal: 10 points
    hurst_points=10 if hcur>.55 else (6 if hcur>=.45 else 4)

    # Volatility context: 10 points. Positive VRP and non-elevated IV get modest risk-on points.
    ...
    elif score>=45: label="NEUTRAL / WAIT"
    elif score>=30: label="BEARISH"
    else: label="STRONG BEARISH"

    reasons=[]
    reasons.append(f"NIFTY: {'above' if n.iloc[-1]>ne20 else 'below'} 20-EMA and {'20-EMA above' if ne20>ne50 else '20-EMA below'} 50-EMA.")
    reasons.append(f"BANK NIFTY: {'bullish confirmation' if (b.iloc[-1]>be20 and be20>be50) else 'not fully confirming the NIFTY trend'}.")
    reasons.append(f"Correlation r={corrcur:.2f}: {'good cross-index confirmation' if corrcur>=.80 else 'divergence reduces conviction' if corrcur<.50 else 'moderate confirmation'}.")
    reasons.append(f"Liquidity: {lreg.lower()}.")
    reasons.append(f"Volatility: IVR {ivr:.1f}% and VRP {vrpcur:+.2f}% ({'option premium above realized vol' if vrpcur>0 else 'option premium below realized vol'}).")
    reasons.append(f"Hurst {hcur:.3f}: {('trend-friendly' if hcur>.55 else 'mean-reverting' if hcur<.45 else 'mixed/random')} regime.")
    return score,label,reasons,dict(trend=trend_points,bank=bank_points,correlation=corr_points,liquidity=liq_points,hurst=hurst_points,volatility=vol_points)
...
_,lreg=liquidity()
reg_score,reg_label,reg_reasons,reg_parts=regime_score(nifty,bank,ivr,vrpcur,hcur,corrcur,lreg or "PRICE DISCOVERY PHASE")
...
with score_col:
    ...
    st.caption("Directional score: trend + cross-index confirmation + liquidity. Volatility metrics are context, not standalone buy/sell signals.")
with why_col:
...
with st.expander("Score breakdown"):
    bd=pd.DataFrame({"Component":["NIFTY trend","BANK NIFTY","Correlation","Liquidity","Hurst regime","Volatility context"],"Points":[reg_parts["trend"],reg_parts["bank"],reg_parts["correlation"],reg_parts["liquidity"],reg_parts["hurst"],reg_parts["volatility"]]})
...
with tabs[0]:
    ...
    x3.metric("Liquidity",lreg or "N/A")
    a,b,c=st.columns(3)
...
with tabs[3]:
    ...
    if ld.empty:
        ...
        st.plotly_chart(fig,use_container_width=True)
        st.metric("Current Microstructure Regime",lreg_tab)

...
def market_decision(nifty, score):
    ...
    # Avoid initiating a fresh trade after an already extended move.
    extension_atr = abs(spot - ema20) / atr if atr > 0 else np.inf
    ...
    if score >= 60 and bullish_structure and not too_extended:
        ...
        reason = (
            "Regime score, EMA trend and volatility extension all support a long setup. "
            f"Breakout confirmation: close above {trigger_level:,.0f}."
        )
        ...
        reason = (
            "Regime score, EMA trend and volatility extension all support a short setup. "
            f"Breakdown confirmation: close below {trigger_level:,.0f}."
        )
        ...
        if too_extended and bearish_structure and score <= 44:
            reason = (
                ...
                "Avoid chasing; wait for a pullback/retest and fresh breakdown confirmation."
            )
            ...
            reason = (
                ...
                "Avoid chasing; wait for a pullback/retest and fresh breakout confirmation."
            )
        elif 45 <= score < 60:
    ...
    return (
        ...
        entry_low, entry_high, sl, t1, t2, reason
    )


def signal_details(decision, score, spot, entry_low, entry_high, sl, t1, t2):
    # R:R is calculated from the actual current/planned spot price.
    # This keeps the displayed R:R mathematically consistent with
    # the ATR-based SL/T1/T2 levels generated by market_decision().
    if decision == "BUY":
        strength = "STRONG BUY" if score >= 75 else "MODERATE BUY"
        trigger = f"BUY confirmation: NIFTY closes above {entry_high:,.0f}"
        risk = spot - sl
        reward_t1 = t1 - spot
        reward_t2 = t2 - spot
        ...
        trigger = f"SHORT confirmation: NIFTY closes below {entry_low:,.0f}"
        risk = sl - spot
        reward_t1 = spot - t1
        reward_t2 = spot - t2
...
with tabs[4]:
    ...
    if key and token:
        try:
            ...
            for k in strikes:
                call_pain = (
                    ((k - strike_array) * calls.values)
                    .clip(min=0)
                    .sum()
                )
                put_pain = (
                    ((strike_array - k) * puts.values)
                    .clip(min=0)
                    .sum()
                )

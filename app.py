"""
NIFTY 50 DATA-DRIVEN INSTITUTIONAL DASHBOARD — V6.1 FINAL
...
def flat(df):
    ...
    # Yahoo can return a trailing row with NaN OHLC (incomplete/live candle,
    # or a data-provider gap). Drop any row missing Open/High/Low/Close so
    # every downstream "last row" calc (spot, EMA, ATR, vol-cone current)
    # always sees a real value instead of nan.
...
def expected_move(nifty,vix):
    ...
    spot=float(c.iloc[-1]); vv=float(vc.iloc[-1])
    # Trading-day scaling (252), matching HV/ATR/Vol-Cone elsewhere in this file.
    # The previous /365 (calendar-day) scaling under-sized this by ~20% relative
    # to every other volatility measure in the engine, which fed directly into
    # the t1/t2 target caps and the mean-reversion entry/target bounds.
    move=spot*(vv/100)*np.sqrt(1/252)
...
def atr14(nifty):
    ...
    # Wilder's smoothing (the original 1978 ATR definition, and what every
    # charting platform -- TradingView, Zerodha Kite -- actually displays).
    # A plain rolling mean drops old spikes off a cliff 14 days later; Wilder's
    # exponential smoothing decays them gradually, which matters here because
    # ATR directly sizes the stop-loss, targets, and the anti-chase distance.
    period=14
    first=tr.rolling(period).mean()
    out=tr.copy()*np.nan
    idx0=period-1
    if len(tr)>idx0:
        out.iloc[idx0]=first.iloc[idx0]
        for i in range(idx0+1,len(tr)):
...
def signal_engine(nifty,bank,vix,iv,vr,h,em,corr,cone,pk,liq,oi,
                   trend_score_threshold=55, mr_score_threshold=10,
                   quality_gate=60, ext_max=1.0, rr_min=1.2):
    ...
    # Trend-Pullback only applies when the regime itself isn't mean-reverting
    # (Hurst<0.45) -- otherwise a coincidentally strong EMA/score reading
    # during a genuinely mean-reverting stretch silently starves the
    # Mean-Reversion branch below, which the if/elif structure would
    # otherwise never reach. Backtest evidence: 5 real historical days
    # (2017-11-07, 2017-12-15, 2018-06-01, 2019-05-23, 2024-03-11) had
    # Hurst<0.45 yet fired a Trend-Pullback trade anyway before this fix.
    not_mean_reverting = h["hurst"]>=.45
    trend_candidate = ((score>=trend_score_threshold and bullish) or (score<=-trend_score_threshold and bearish)) and not_mean_reverting
    decision="WAIT"; entry_low=entry_high=trigger=sl=t1=t2=np.nan; setup_type="NO TRADE"
    if trend_candidate and ext<=ext_max and quality>=quality_gate:
    ...
    elif h["hurst"]<.45 and quality>=quality_gate and liq.get("available") and (liq.get("demand") or liq.get("supply")):
        # Mean-reversion entry is only allowed after a confirmed sweep near the expected-move boundary.
        if liq.get("demand") and spot<=em["upper"] and score>=mr_score_threshold:
            decision="BUY"; setup_type="MEAN-REVERSION BUY"; entry_low=max(em["lower"],liq.get("prev_low",em["lower"])); entry_high=spot; trigger=spot; sl=(liq.get("prev_low") or em["lower"])-.35*atr; t1=spot+0.5*(em["upper"]-spot); t2=em["upper"]; reasons.append("Mean-reversion setup: demand sweep is confirmed; buy only after price closes back above the swept level.")
        elif liq.get("supply") and spot>=em["lower"] and score<=-mr_score_threshold:
            decision="SHORT"; setup_type="MEAN-REVERSION SHORT"; entry_low=spot; entry_high=min(em["upper"],liq.get("prev_high",em["upper"])); trigger=spot; sl=(liq.get("prev_high") or em["upper"])+.35*atr; t1=spot-0.5*(spot-em["lower"]); t2=em["lower"]; reasons.append("Mean-reversion setup: supply sweep is confirmed; short only after price closes below the swept level.")
    ...
    # Geometry gate: the daily "spot" (EOD close) and the 15m liquidity levels
    # (Prev_High/Prev_Low) come from two different feeds/timeframes and can
    # disagree enough that entry_low ends up ABOVE entry_high -- a nonsensical
    # entry zone. The R:R gate below catches most of these by coincidence
    # (a negative risk forces R:R to NaN), but not all of them, so check
    # explicitly rather than relying on that side effect.
    if decision!="WAIT" and np.isfinite(entry_low) and np.isfinite(entry_high) and entry_low>entry_high:
        reasons.append("Entry zone geometry invalid (daily close vs 15m liquidity level mismatch); downgraded to WAIT.")
    ...
    if decision!="WAIT" and (not np.isfinite(rr1) or rr1<rr_min):
        reasons.append(f"R:R gate failed: T1 does not offer at least {rr_min:.2f}R from the planned entry, so the trade is downgraded to WAIT.")
...
# ---------------- UI ----------------
def main():
    st.title("📊 NIFTY 50 Data-Driven Institutional Dashboard — V6.1 FINAL")
    ...
    # Data-integrity guard: never generate a trading score from incomplete core data.
    core_ok = all([
    ...
    try:
        ...
        st.error(f"Signal engine could not run: {e}")
        st.stop()

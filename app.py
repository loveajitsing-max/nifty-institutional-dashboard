"""
NIFTY 50 DATA-DRIVEN INSTITUTIONAL DASHBOARD — V6.1
...
st.set_page_config(page_title="NIFTY Data-Driven V6.1 FINAL", page_icon="📊", layout="wide")
...
def vol_cone(nifty):
    ...
    for w in wins:
        ...
        valid=rv.dropna()
        out["max"].append(float(valid.max()) if not valid.empty else np.nan)
        out["min"].append(float(valid.min()) if not valid.empty else np.nan)
        out["median"].append(float(valid.median()) if not valid.empty else np.nan)
        out["current"].append(float(valid.iloc[-1]) if not valid.empty else np.nan)
...
def effective_risk_distance(spot, atr, parkinson_ann):
    # Parkinson is annualized; convert it to an approximate daily point risk floor.
    park_daily_pts = spot * float(parkinson_ann) / np.sqrt(252) if np.isfinite(parkinson_ann) else 0.0
    return float(max(atr, 0.75 * park_daily_pts))

def ema_levels(nifty):
    c=col(nifty,"Close").dropna()
    if len(c)<200:
        raise ValueError("NIFTY daily Close data is insufficient for EMA200.")
    e20=c.ewm(span=20,adjust=False).mean()
    e50=c.ewm(span=50,adjust=False).mean()
    e200=c.ewm(span=200,adjust=False).mean()
    return float(c.iloc[-1]),float(e20.iloc[-1]),float(e50.iloc[-1]),float(e200.iloc[-1])
...
def signal_engine(nifty,bank,vix,iv,vr,h,em,corr,cone,pk,liq,oi):
    ...
    # V6.1: directional evidence is kept separate from non-directional quality/risk evidence.
    bullish=spot>ema20>ema50 and ema50>ema200
    bearish=spot<ema20<ema50 and ema50<ema200
    mom10=float((spot/close.iloc[-11]-1)*100) if len(close)>11 else 0

    if bullish:
        score+=30; reasons.append("NIFTY EMA structure is bullish (20 > 50 > 200).")
    elif bearish:
        score-=30; reasons.append("NIFTY EMA structure is bearish (20 < 50 < 200).")
    else:
        quality-=10; reasons.append("NIFTY EMA structure is mixed; directional conviction reduced.")

    if h["hurst"]>.55:
        if mom10>0:
            score+=20; reasons.append(f"Hurst {h['hurst']:.3f} is TRENDING and 10D momentum is positive.")
        else:
            score-=20; reasons.append(f"Hurst {h['hurst']:.3f} is TRENDING and 10D momentum is negative.")
    elif h["hurst"]<.45:
        quality-=15; reasons.append(f"Hurst {h['hurst']:.3f} is MEAN REVERTING; breakout chasing is filtered.")
        if liq.get("demand"):
            score+=18; reasons.append("15m demand sweep supports a mean-reversion long.")
        if liq.get("supply"):
            score-=18; reasons.append("15m supply sweep supports a mean-reversion short.")
    else:
        quality-=8; reasons.append(f"Hurst {h['hurst']:.3f} is RANDOM WALK; no strong trend edge.")

    # Bank Nifty is directional confirmation only when correlation is high.
    if corr["corr"]>.80:
        bclose=float(col(bank,"Close").iloc[-1])
        b20=float(col(bank,"Close").ewm(span=20,adjust=False).mean().iloc[-1])
        b50=float(col(bank,"Close").ewm(span=50,adjust=False).mean().iloc[-1])
        if bclose>b20>b50:
            score+=15; reasons.append("Bank Nifty confirms the bullish direction with high correlation.")
        elif bclose<b20<b50:
            score-=15; reasons.append("Bank Nifty confirms the bearish direction with high correlation.")
        else:
            quality-=8; reasons.append("High Nifty–Bank correlation, but Bank Nifty trend is not confirming.")
    elif corr["corr"]<.50:
        quality-=18; reasons.append(f"Nifty–Bank correlation {corr['corr']:.2f}: severe divergence, so conviction is reduced.")
    else:
        quality-=8; reasons.append(f"Nifty–Bank correlation {corr['corr']:.2f}: moderate divergence.")

    # ---------------- Non-directional quality/risk layer ----------------
    ivr=iv.get("ivr",np.nan); vrpv=vr.get("vrp",np.nan)
    if np.isfinite(ivr) and np.isfinite(vrpv):
        if ivr>50 and vrpv>0:
            quality-=5
            reasons.append(f"IVR {ivr:.1f}% + VRP {vrpv:+.2f}%: volatility premium is rich; trade quality slightly reduced.")
        elif ivr<50 and vrpv<0:
            reasons.append(f"IVR {ivr:.1f}% + VRP {vrpv:+.2f}%: volatility premium is relatively cheap; no directional boost applied.")
        else:
            reasons.append(f"IVR {ivr:.1f}% / VRP {vrpv:+.2f}%: mixed volatility context; no directional score applied.")

    cone_ratio=cone["current"][1]/cone["median"][1] if np.isfinite(cone["current"][1]) and np.isfinite(cone["median"][1]) and cone["median"][1]!=0 else 1
    if cone_ratio>1.25:
        quality-=10; reasons.append("20D realized volatility is well above its cone median; risk is elevated.")
    elif cone_ratio<.75:
        quality-=5; reasons.append("20D realized volatility is below its cone median; breakout follow-through needs confirmation.")

    if pk["parkinson"]>pk["c2c"]:
        quality-=5; reasons.append("Parkinson volatility exceeds close-to-close volatility: intraday range risk is elevated.")

    if liq.get("available"):
        if liq.get("demand") and score<0:
            quality-=10; reasons.append("Demand sweep conflicts with bearish direction.")
        if liq.get("supply") and score>0:
            quality-=10; reasons.append("Supply sweep conflicts with bullish direction.")
    ...
    decision="WAIT"; entry_low=entry_high=trigger=sl=t1=t2=np.nan
    setup_type="NO TRADE"; risk_dist=atr

    if trend_candidate and ext<=1.0 and quality>=60:
        half=.25*atr
        entry_low=ema20-half; entry_high=ema20+half

        # Expected Move is a feasibility/target boundary, not a directional signal.
        if entry_low<=em["upper"] and entry_high>=em["lower"]:
            risk_dist=effective_risk_distance(spot,atr,pk["parkinson"])

            if score>0:
                decision="BUY"; setup_type="TREND PULLBACK BUY"
                trigger=max(entry_high,prev5h)
                sl=entry_low-risk_dist
                t1=min(ema20+1.5*risk_dist,em["upper"])
                t2=min(ema20+2.5*risk_dist,em["upper"]+0.5*em["move"])

                cw=oi.get("call_wall",np.nan) if oi.get("available") else np.nan
                planned=(entry_low+entry_high)/2
                if np.isfinite(cw):
                    if cw<=trigger:
                        quality-=20
                        reasons.append(f"Call wall {cw:.0f} is at/below the BUY trigger; upside OI resistance conflicts with the setup.")
                    elif cw-planned<risk_dist:
                        quality-=12
                        reasons.append(f"Call wall {cw:.0f} is too close to the planned BUY entry; resistance is less than 1 effective risk unit away.")
                    t1=min(t1,cw)
                reasons.append("Trend setup: wait for pullback into EMA20 zone, then daily close above confirmation trigger.")
            else:
                decision="SHORT"; setup_type="TREND PULLBACK SHORT"
                trigger=min(entry_low,prev5l)
                sl=entry_high+risk_dist
                t1=max(ema20-1.5*risk_dist,em["lower"])
                t2=max(ema20-2.5*risk_dist,em["lower"]-0.5*em["move"])

                pw=oi.get("put_wall",np.nan) if oi.get("available") else np.nan
                planned=(entry_low+entry_high)/2
                if np.isfinite(pw):
                    if pw>=trigger:
                        quality-=20
                        reasons.append(f"Put wall {pw:.0f} is at/above the SHORT trigger; downside OI support conflicts with the setup.")
                    elif planned-pw<risk_dist:
                        quality-=12
                        reasons.append(f"Put wall {pw:.0f} is too close to the planned SHORT entry; support is less than 1 effective risk unit away.")
                    t1=max(t1,pw)
                reasons.append("Trend setup: wait for pullback into EMA20 zone, then daily close below confirmation trigger.")
        else:
            reasons.append("EMA20 entry zone does not overlap the current 1D Expected Move; trend setup blocked.")
        ...
        if liq.get("demand") and spot<=em["upper"] and score>=10:
            decision="BUY"; setup_type="MEAN-REVERSION BUY"
            entry_low=max(em["lower"],liq.get("prev_low",em["lower"])); entry_high=spot; trigger=spot
            risk_dist=effective_risk_distance(spot,atr,pk["parkinson"])
            sl=(liq.get("prev_low") or em["lower"])-.35*risk_dist
            t1=min(em["spot"],em["upper"]); t2=em["upper"]
            reasons.append("Mean-reversion setup: demand sweep is confirmed; buy only after price closes back above the swept level.")
        elif liq.get("supply") and spot>=em["lower"] and score<=-10:
            decision="SHORT"; setup_type="MEAN-REVERSION SHORT"
            entry_low=spot; entry_high=min(em["upper"],liq.get("prev_high",em["upper"])); trigger=spot
            risk_dist=effective_risk_distance(spot,atr,pk["parkinson"])
            sl=(liq.get("prev_high") or em["upper"])+.35*risk_dist
            t1=max(em["spot"],em["lower"]); t2=em["lower"]
            reasons.append("Mean-reversion setup: supply sweep is confirmed; short only after price closes back below the swept level.")
    ...
    # OI conflict and quality gate.
    if decision!="WAIT" and quality<60:
        reasons.append("Setup quality fell below 60 after volatility/OI conflict checks; trade downgraded to WAIT.")
        decision="WAIT"; setup_type="NO TRADE"; entry_low=entry_high=trigger=sl=t1=t2=np.nan

    planned=(entry_low+entry_high)/2 if np.isfinite(entry_low) and np.isfinite(entry_high) else np.nan
    if decision=="BUY":
        risk=planned-sl; rr1=(t1-planned)/risk if risk>0 else np.nan; rr2=(t2-planned)/risk if risk>0 else np.nan
    elif decision=="SHORT":
        risk=sl-planned; rr1=(planned-t1)/risk if risk>0 else np.nan; rr2=(planned-t2)/risk if risk>0 else np.nan
    else:
        rr1=rr2=np.nan
    ...
    breakeven=planned if decision!="WAIT" else np.nan
    trail=(t1-risk_dist if decision=="BUY" else t1+risk_dist if decision=="SHORT" else np.nan)

    return {"decision":decision,"setup":setup_type,"score":score,"quality":max(0,quality),
            "confidence":confidence,"spot":spot,"ema20":ema20,"ema50":ema50,"ema200":ema200,
            "atr":atr,"risk_dist":risk_dist,"entry_low":entry_low,"entry_high":entry_high,
            "trigger":trigger,"sl":sl,"t1":t1,"t2":t2,"rr1":rr1,"rr2":rr2,
            "breakeven":breakeven,"trail":trail,"reasons":reasons,"ext":ext}

# ---------------- UI ----------------
def main():
    st.title("📊 NIFTY 50 Data-Driven Institutional Dashboard — V6.1.1.1")
    ...
    iv=ivr_ivp(vix); vr=vrp(nifty,vix); h=hurst(nifty); em=expected_move(nifty,vix); co=correlation(nifty,bank); cone=vol_cone(nifty); pk=parkinson(nifty); liq=liquidity(n15); oi=oi_profile_from_kite()
    # Never generate a trading score from incomplete core data.
    core_ok = all([
        np.isfinite(em.get("spot",np.nan)),
        np.isfinite(iv.get("current",np.nan)),
        np.isfinite(h.get("hurst",np.nan)),
        np.isfinite(co.get("corr",np.nan)),
        np.isfinite(pk.get("parkinson",np.nan)),
        np.isfinite(pk.get("c2c",np.nan))
    ])
    if not core_ok:
        st.error("Core market data is incomplete. Trading signal is disabled until valid NIFTY/VIX data is available.")
        st.stop()
    sig=signal_engine(nifty,bank,vix,iv,vr,h,em,co,cone,pk,liq,oi)
    ...
    if sig['decision']!="WAIT":
        ...
        st.caption(f"Effective risk unit: {sig['risk_dist']:,.1f} pts (ATR/Parkinson floor). After T1: protect the position around breakeven {sig['breakeven']:,.0f} and trail the remainder.")
